...
function getCheckboxValue()  {
  ...
  // 첫번째 관광지 랜덤 선택
  var rand_first_area = '';
  rand_first_area = checkvl[Math.floor(Math.random()*10)];
  //document.getElementById('result').innerHTML = tourarr;
  ...
  for(var n = 0; n < addr2.length; n++){
    ...
    if(dis_srh == 'di') {
      ...
      // 랭킹 순으로 정렬
      for(var hh = 0; hh < result.length; hh++) {
  ...
  // 길찾기 버튼
  /*
  const text1 = document.getElementById('howgo');
  const btn1 = document.getElementById('direct_1');
  const btn2 = document.getElementById('direct_2');
  const btn3 = document.getElementById('direct_3');
  const btn4 = document.getElementById('direct_4');
*/
  ...
  while(qq != cnt-1) {
    ...
    button.setAttribute('onclick', "directB(this)");
