...
<div class="ing_bnr_Wrap">
    ...
    <div class="bnrtxtwrap">
        ...
        <p class="wow fadeInDown" data-wow-delay="0.2s">투어리즘에 등재된 국내 숙박업소 목록입니다.</p>
    </div>

</div>
<div class="mdlTxt">
    <h2 class="wow flipInX" data-wow-duration="2s">국내 숙박업소 목록</h2>
</div>
...
<?php
include_once('./hotel_local_select.php');
?>
    ...
    <div class="inner">
            ...
            //$local = '<script> document.write(anText); </script>';
              ...
              class Pagination {
                  ...
                  $prev_block,
                  $prev_block_page,
                  $PHP_SELF = "https://idox23.cafe24.com/theme/AT_WEB01/company/tour_list/kr_hotel/kr_hotel_list.php";
                  ...
                  public function __construct($pg, $bl, $dbName) {
                      ...
                      $query = "SELECT count(name) AS total FROM kr_hotel";
              ...
              $pagination = new Pagination(10, 10, $dbName);

              //데이터 로드
              $query = "SELECT * FROM kr_hotel ORDER BY ranking LIMIT " . $pagination->limit_idx . ", " . $pagination->page_set;
              $stmt = $db->prepare($query);
              $stmt->execute();
              ...
              <ul>
                ...
                <script>
                  ...
                  function send_data(ths){
                    ...
                    var value = world1[1];
                    //alert('값 : ' + value);
                    location.href = "./kr_hotel_info.php?value=" + value;
                    //location.href = "../kr_tour_info.php?value=" + encodeURI(encodeURIComponent(value));
                  }
                </script>
                <li onclick="send_data(this)" class="tour_list">
                  <div class="image_box">
                    <img src="<?php echo $row['firstimage']; ?>" width=90% height=100% alt>
                  </div>
                  <div class="tour_box">
                    <dl>
                      <dt class="name">
                        <?php
                          echo '<p id="hide">@</p>'.$row['name'].'<p id="hide">@</p>';
                        ?>
                        <p class="ranking">
                          <?php
                            echo $row['ranking'];
