...
$tour_name = $_GET["value"];
  ...
  //데이터 로드
  $query = "SELECT * FROM kr_camp WHERE name='" . $tour_name . "'";
...
<div class="wrap_contView">
  ...
  <div class="inr_wrap">
    <ul class="inr_info_wrap">
      <li class="inr_info">
        ...
        <span class="info_text"> <?php echo $row['infocenterleports']; ?> </span>
      </li>
      <li class="inr_info">
        <strong>■ 휴무일 : </strong>
        <span class="info_text"> <?php echo $row['restdateleports']; ?> </span>
      </li>
      <li class="inr_info">
        <strong>■ 이용시간 : </strong>
        <span class="info_text"> <?php echo $row['usetimeleports']; ?> </span>
      </li>
			<li class="inr_info">
        <strong>■ 예약 안내 : </strong>
        <span class="info_text"> <?php echo $row['reservation']; ?> </span>
      </li>
			<li class="inr_info">
        <strong>■ 주차 시설 : </strong>
        <span class="info_text"> <?php echo $row['parkingleports']; ?> </span>
      </li>
      <li class="inr_info">
        <strong>■ 반려동물 여부 : </strong>
        <span class="info_text"> <?php echo $row['chkpetleports']; ?> </span>
      </li>
      <li class="inr_info">
        <strong>■ 이용 요금 : </strong>
        <span class="info_text"> <?php echo $row['fee']; ?> </span>
      </li>
      <li class="inr_info">
        <strong>■ 주요 시설 : </strong>
        <span class="info_text"> <?php echo $row['facility']; ?> </span>
      </li>
    </ul>
  </div>
  ...
  <h3 class="tour_info_title"> 가까운 거리의 관광지 추천 </h3>
	...
	FROM kr_camp WHERE areacode='".$row['areacode']."' ORDER BY distance";
	$stmt2 = $db->prepare($q2);
	$stmt2->execute();
	$re2 = $stmt2->fetchAll();
	$arr = array();
	...
	function send_data(ths){
	  var str = $(ths).text();
	  location.href = "/theme/AT_WEB01/company/tour_best/kr_camp_info.php?value=" + str;
	}
