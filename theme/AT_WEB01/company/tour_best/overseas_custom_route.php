...
while ($row = mysqli_fetch_array($re)) {
  array_push($arr, ['ha' => [$row['name'] => $row['lng'].'/'.$row['lat']]]);
}
while ($row2 = mysqli_fetch_array($re2)) {
  array_push($arr, ['la' => [$row2['name'] => $row2['lng'].'/'.$row2['lat']]]);
}
while ($row3 = mysqli_fetch_array($re3)) {
  array_push($arr, ['lo' => [$row3['name'] => $row3['lng'].'/'.$row3['lat']]]);
}
while ($row4 = mysqli_fetch_array($re4)) {
  array_push($arr, ['ne' => [$row4['name'] => $row4['lng'].'/'.$row4['lat']]]);
}
while ($row5 = mysqli_fetch_array($re5)) {
  array_push($arr, ['sa' => [$row5['name'] => $row5['lng'].'/'.$row5['lat']]]);
}
...
<div class="wrap_contView">
  ...
  <table class="custom" align="center">
    ...
    <tr>
      <th> 검색 기준 </th>
      <td colspan="7"><p><label><input type="checkbox" id="dis_search" name="dis_search" value="on" checked> 거리순으로 검색하기 </label></p></td>
    </tr>
  </table>
...
function getCheckboxValue()  {
  ...
  // 첫번째 관광지 랜덤 선택
  var rand_first_area = '';
  rand_first_area = tourarr[Math.floor(Math.random()*(tourarr.length-1))];
  ...
  for(var n = 0; n < addr2.length; n++){
    ...
    if(d < distance) {
      di_sort.push(d);
      result.push(tourarr[n]); // 선택한 거리 이내에 존재하는 관광지 이름(1번빼고)
  ...
  //document.getElementById('result').innerHTML = addr2_re;
...
function send_data(ths){
  ...
  var id = $(ths).attr('id');
  if(id == 'ha') { // 하와이
    location.href = "./ov_tour_info.php?loc=1&value=" + str;
